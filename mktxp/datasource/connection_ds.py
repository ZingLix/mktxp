    ...
    def metric_records(router_entry, *, metric_labels=None):
        ...
        try:
            answer = (
                ...
                .get_binary_resource("/ip/firewall/connection/")
                .call("print", {"count-only": b""})
            )
            ...
            return BaseDSProcessor.trimmed_records(
                router_entry, router_records=records, metric_labels=metric_labels
            )
        except Exception as exc:
            print(
                f"Error getting IP connection info from router{router_entry.router_name}@{router_entry.config_entry.hostname}: {exc}"
            )
            return None
...
class IPConnectionStatsDatasource:
    ...
    def metric_records(router_entry, *, metric_labels=None, add_router_id=True):
        ...
        try:
            connection_records = (
                router_entry.api_connection.router_api()
                .get_resource("/ip/firewall/connection/")
                .call("print", {"proplist": "src-address,dst-address,protocol"})
            )
            ...
            for connection_record in connection_records:
                ...
                connections_per_src_address[address] = ConnStatsEntry(
                    count, destinations
                )
            ...
            for key, entry in connections_per_src_address.items():
                record = {
                    ...
                    "dst_addresses": ", ".join(entry.destinations),
                }
                if add_router_id:
            ...
            print(
                f"Error getting IP connection stats info from router{router_entry.router_name}@{router_entry.config_entry.hostname}: {exc}"
            )
            return None


class IPConnectionRateDatasource:
    """ IP connections stats data provider
    """
        ...
        try:
            connection_records = (
                ...
                .call(
                    "print",
                    {
                        "proplist": "src-address,dst-address,orig-rate,repl-rate,orig-bytes,repl-bytes,protocol"
                    },
                )
            )
            ...
            for item in connection_records:
                record = item
                record["src_ip"] = item["src-address"].split(":")[0]
                record["src_address"] = item["src-address"]
                record["dst_address"] = item["dst-address"]
                record["repl-rate"] = int(item["repl-rate"])
                record["orig-rate"] = int(item["orig-rate"])
                record["orig-bytes"] = int(item["orig-bytes"])
                record["repl-bytes"] = int(item["repl-bytes"])
                if add_router_id:
                    for router_key, router_value in router_entry.router_id.items():
                        record[router_key] = router_value
                records.append(record)
